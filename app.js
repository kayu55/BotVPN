...
async function detectServerIsp(domain) {
  ...
  try {
    ...
    const response = await axios.get(`http://ip-api.com/json/${encodeURIComponent(address)}?fields=status,isp`);
    const data = response.data;
    if (data && data.status === 'success') {
      ispCache.set(domain, data.isp || null);
      return data.isp || null;
    }
  } catch (err) {
    logger.warn(`Gagal mendeteksi ISP untuk domain ${domain}: ${err.message}`);
  }
...
bot.command('paymet', async (ctx) => {
  ...
  if (!['GOPAY', 'ORKUT'].includes(arg)) {
    return ctx.reply('⚠️ Pilihan tidak valid. Gunakan GOPAY atau ORKUT');
  }
...
async function handleServiceAction(ctx, action) {
  ...
  if (action === 'create') {
    keyboard = [
      [{ text: 'Buat Ssh/Ovpn/Udp', callback_data: 'create_ssh' }],      
    ...
    keyboard = [
      [{ text: 'Trial Ssh/Ovpn/Udp', callback_data: 'trial_ssh' }],      
    ...
    keyboard = [
      [{ text: 'Perpanjang Ssh/Ovpn/Udp', callback_data: 'renew_ssh' }],      
    ...
    keyboard = [
      [{ text: 'Hapus Ssh/Ovpn/Udp', callback_data: 'del_ssh' }],      
    ...
    keyboard = [
      [{ text: 'Lock Ssh/Ovpn/Udp', callback_data: 'lock_ssh' }],      
    ...
    keyboard = [
      [{ text: 'Unlock Ssh/Ovpn/Udp', callback_data: 'unlock_ssh' }],      
    ...
    keyboard = [
      [{ text: 'Change Limit Ssh/Ovpn/Udp', callback_data: 'changelimip_ssh' }],      
    ...
    keyboard = [
      [{ text: 'Fix Ssh/Ovpn/Udp', callback_data: 'fix_ssh' }],      
...
bot.action('jadi_reseller', async (ctx) => {
  ...
  db.get('SELECT saldo FROM users WHERE user_id = ?', [userId], async (err, row) => {
    ...
    // Verifikasi otomatis jika saldo >= 100k
  ...
  try {
    if (vars.PAYMENT === 'ORKUT') {
      ...
      const res = await axios.post(
        ...
        params,
        {
          headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Accept': '*/*', 'User-Agent': 'Mozilla/5.0 (compatible; Bot/1.0) ' },
          timeout: 15000
        }
      );
    ...
    if (err.response) {
      ...
      const body = err.response.data ? (typeof err.response.data === 'string' ? err.response.data : JSON.stringify(err.response.data)) : '';
      return ctx.reply(`❌ Gagal cek pembayaran: status ${status}\n${body}`);
    }

    logger.error('Error cek_topup: ' + err.message);
    return ctx.reply('❌ Terjadi error saat cek pembayaran: ' + err.message);
  }
});
    ...
    db.all(query, [], async (err, servers) => {
      ...
      let filteredServers = servers.filter(server => {
        ...
        if (isR) {
          ...
          if (!isResellerOnly) {
            logger.info(`Menyembunyikan server ${server.nama_server} (regular) untuk reseller ${ctx.from.id}`);
            return false;
          }
          return true;
        } else {
          ...
          if (isResellerOnly) {
            logger.info(`Menyembunyikan server ${server.nama_server} (reseller) untuk user biasa ${ctx.from.id}`);
            return false;
          }
          return true;
        }
      ...
      for (const server of currentServers) {
        ...
        const ispInfo = await detectServerIsp(server.domain);
        const ispLine = ispInfo ? `📡 ISP: ${ispInfo}\n` : '';

        serverListItems.push(`🌐 *${server.nama_server}*\n` +
               ispLine +
      ...
      const serverList = serverListItems.join('\n\n');
