...
if (!/^[a-z0-9-]+$/.test(username)) {
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      const param = `:5888/addss?user=${username}&exp=${exp}&quota=${quota}&iplimit=${limitip}&auth=${auth}`;
